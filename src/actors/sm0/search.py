from .typers import *
from actors.sm0.config import StochasticMuZeroConfig
from game_utils import get_opp
import numpy as np
import math
...
class ActionOutcomeHistory:
  ...
  """
    ...
    def __init__(self, player: Color, history: List[np.array] = None):
        self.initial_player = player
        self.history = list(history or [])
        self.player = player

    def clone(self):
        return ActionOutcomeHistory(self.player, self.history)

    def add_action_or_outcome(self, action_or_outcome: ActionOrOutcome):
        self.history.append(action_or_outcome)

    def swap_player(self):
        self.player = get_opp(self.player)

    def last_action_or_outcome(self):
...
def run_mcts(config: StochasticMuZeroConfig, root: Node,
             action_outcome_history: ActionOutcomeHistory, network: tf.keras.Model,
             min_max_stats: MinMaxStats, verbose: bool = False):
    pbar = range(config.num_simulations)
    if verbose:
        pbar = tqdm(pbar)
    ...
    for _ in pbar:
        ...
        if verbose:
            pbar.set_description(f"Value: {root.value_sum:.4f}")
        ...
        if parent.is_chance:
            ...
            child_state, _ = network.dynamics(parent.state,
...
# the neural network.
def expand_node(node: Node, state: np.array,
...
def add_exploration_noise(config: StochasticMuZeroConfig, node: Node):
    ...
    frac = config.root_dirichlet_fraction
