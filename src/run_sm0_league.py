...
from actors.sm0.config import sequence_sm0_config
from actors.sm0 import StochasticMuZeroActor
from algorithms.sm0 import StochasticMuZeroAlgorithm
from networks.sm0 import StochasticMuZeroNetwork
...
def run_n_selfplay(
        n: int, actor: Actor,
        replay_buffer: RolloutReplayBuffer, max_turns: int = 1000):
    for _ in range(n):
...
def recreate_league_from_dir(league_dir, config):
    ...
    for lid in L.players:
        ...
        new_actor = create_sm0_bot(new_actor_weights, config)
...
def create_sm0_bot(weight_file, config):
    net = StochasticMuZeroNetwork()
    algo = StochasticMuZeroAlgorithm(config, net, learning_rate=config.learning_rate)
    algo.build()
    del algo.optimizer
    algo.model.load_weights(weight_file)
    actor = StochasticMuZeroActor(config, algo, training=False)
    return actor
...
def main(model_path, league_dir, init_weights, resume):
    ...
    actor = StochasticMuZeroActor(config, algo, training=True)
    replay_buffer = RolloutReplayBuffer(config)
